...
static void bench_working_set(size_t iters, size_t set_size, size_t max_sz)
{
    ...
    if (!set) { fprintf(stderr, "set alloc failed\n"); return; }
    uint64_t seed = 0xCAFEBABEDEADBEEFULL;
    double t0 = now_sec();
    for (size_t i=0;i<iters;++i) {
        size_t idx = xorshift64(&seed) % set_size;
        if (set[idx]) { free(set[idx]); set[idx] = NULL; }
        size_t sz = (xorshift64(&seed) % max_sz) + 1;
        set[idx] = malloc(sz);
        touch(set[idx], sz);
    }
    for (size_t i=0;i<set_size;++i) free(set[i]);
    free(set);
    double t1 = now_sec();
...
static void bench_realloc(size_t iters, size_t start_sz, size_t max_sz)
{
    ...
    for (size_t i=0;i<iters;++i) {
        ...
        if (target > cur) {
            ...
            if (n) { p = n; cur = target; }
        } else if ((i & 0x7) == 0) { /* occasional shrink */
            void *n = realloc(p, target);
            if (n) { p = n; cur = target; }
        }
        touch(p, cur);
...
static void bench_fragment(size_t blocks, size_t block_sz)
{
    ...
    if (!arr) { fprintf(stderr, "fragment arr alloc fail\n"); return; }
    for (size_t i=0;i<blocks;++i) { arr[i] = malloc(block_sz); touch(arr[i], block_sz); }
    /* free every other */
    for (size_t i=0;i<blocks;i+=2) { free(arr[i]); arr[i] = NULL; }
    /* allocate new blocks for freed spots with slightly different size */
    size_t new_sz = block_sz / 2 + 1;
    for (size_t i=0;i<blocks;i+=2) { arr[i] = malloc(new_sz); touch(arr[i], new_sz); }
    /* free all */
    for (size_t i=0;i<blocks;++i) free(arr[i]);
    free(arr);
